...
{
	//设备随时有可能变更，所以每次切换场景时，实时更新下
	tblive_sdk::check_add_all_audio_sources_to_scene(obs_scene_from_source(scene));

	if (!IsPreviewProgramMode()) {
		// Check if it is streaming now
		if (outputHandler->StreamingActive()) {
			// Notify user whether to switch to this scene
			// if switched, it will cause current scene to output to libobs and rtmp
		} else {
			// Not streaming
			TransitionToScene(scene, false);
		}
