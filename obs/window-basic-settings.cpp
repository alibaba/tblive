	  ...
	  ui               (new Ui::OBSBasicSettings)
{
	...
	// basic
	ui->listWidget->item(SETTING_GENERAL)->setHidden(true);
	ui->settingsPages->widget(SETTING_GENERAL)->setVisible(false);

	// hotkey
	ui->listWidget->item(SETTING_HOTKEY)->setHidden(true);
	ui->settingsPages->widget(SETTING_HOTKEY)->setVisible(false);

	// advanced
	ui->listWidget->item(SETTING_ADVANCED)->setHidden(true);
	ui->settingsPages->widget(SETTING_ADVANCED)->setVisible(false);

	ui->listWidget->setCurrentRow(SETTING_STREAM1);
	...
	{
		QListWidgetItem * pItem = ui->listWidget->item(nIndex);
		pItem->setSizeHint(QSize(140, 40));
	}

	//设置当前选择项，需与左侧列表匹配一致，左侧列表是在ui designer中设置的
	ui->settingsPages->setCurrentIndex(1);
}

bool OBSBasicSettings::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
	MSG* msg = (MSG*)message;
	switch (msg->message)
	{
...
void OBSBasicSettings::LoadResolutionLists()
{
	...
	if ( !hasDefault )
	{
		ui->baseResolution->addItem("1280x720");
	}
...
void OBSBasicSettings::LoadAudioSources()
{
	...
	auto AddSource = [&](obs_source_t *source)
	{
		...
		//修复：系统设置窗体中避免出现横向滚动栏，不方便观看
		QString strFullName = label->text();
		QString streElideName = GetElidedText(label->font(), strFullName, 222);
		label->setText(streElideName);
		if (strFullName != streElideName)
		{
			label->setToolTip(strFullName);
		}
...
void OBSBasicSettings::on_pushButton_ok_clicked()
{
	SaveSettings();
	ClearChanged();
	close();
...
void OBSBasicSettings::on_pushButton_apply_clicked()
{
	SaveSettings();
	ClearChanged();
}

void OBSBasicSettings::on_pushButton_cancle_clicked()
{
...
void OBSBasicSettings::on_pushButton_close_clicked()
{
...
void OBSBasicSettings::keyPressEvent(QKeyEvent *event)
{
	...
	{
		QDialog::keyPressEvent(event);	//对于非enter键，调用默认实现
	}
}
