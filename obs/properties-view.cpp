		...
		QLabel **label)
{
	...
	if (std::string("file") == name)
	{
		obs_source_t * cur_source = (obs_source_t *)this->obj;
		if (cur_source)
		{
			std::string source_name = obs_source_get_id(cur_source);
			if (source_name == "image_source")
			{
				isImageFile = true;
			}
		}
	...
	if ( isImageFile )
	{
		edit->setText(QString::fromLocal8Bit( QByteArray::fromStdString(val) ));
	}
	else
	{
		edit->setText(QT_UTF8(val));
...
bool WidgetInfo::PathChanged(const char *setting)
{
	...
	bool isImageFile = false;
	if ( std::string("file") == setting )
	{
		obs_source_t * cur_source = (obs_source_t *)view->obj;
		if (cur_source)
		{
			std::string source_name = obs_source_get_id(cur_source);
			if (source_name == "image_source")
	...
	if ( isImageFile )
	{
		obs_data_set_string(view->settings, setting, path.toLocal8Bit().constData());
	}
	else
	{
