	  ...
	  ui             (new Ui::OBSBasic)
{
    ...
    tblive_sdk::set_obs_basic(this);

	InitHoverVolWidget();
	//设置焦点代理是为了使HoverVolWidget焦点捕获正常工作
...
void OBSBasic::ClearVolumeControls()
{
	...
	//新增vol控制台,这里释放后，会自动从布局器里面清除
	ClearCustomVolumeControls();
}
void OBSBasic::ClearCustomVolumeControls()
{
	//新增vol控制台,这里释放后，会自动从布局器里面清除
	VolControlCustom *volControlCustom;
	for (std::map<OBSSource, VolControlCustom*>::iterator it = m_volumeCustomsMap.begin(); it != m_volumeCustomsMap.end(); it++) 
	{
		volControlCustom = it->second;
		delete volControlCustom;
	}
	m_volumeCustomsMap.clear();
}
...
void OBSBasic::CreateDefaultScene(bool firstStart)
{
	...
	QString defSceneName = format.arg(m_curNewSceneNameIndex++);

	obs_scene_t  *scene = obs_scene_create(QT_TO_UTF8(defSceneName));
...
bool OBSBasic::LoadService()
{
	...
	obs_data_set_default_string(data, "type", "rtmp_custom");
...
void OBSBasic::ReloadServiceFromConfig(std::string json)
{
	if (service) {
		service = nullptr;
	}

	BPtr<char> jsonText = bstrdup_n(json.data(), json.size());
	if (!jsonText) {
		return;
	}
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	config_set_default_uint  (basicConfig, "AdvOut", "Track1Bitrate", 128);
	config_set_default_uint  (basicConfig, "AdvOut", "Track2Bitrate", 128);
	config_set_default_uint  (basicConfig, "AdvOut", "Track3Bitrate", 128);
	config_set_default_uint  (basicConfig, "AdvOut", "Track4Bitrate", 128);

	// Hardcode as 1280x720
	cx = 1280;	cy = 720;
	...
	config_set_default_uint  (basicConfig, "Video", "FPSInt", 25);
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "tblive/basic/scenes/%s.json",
			sceneCollection);
	if (ret <= 0)
...
OBSBasic::~OBSBasic()
{
    tblive_sdk::set_obs_basic(nullptr);
...
void OBSBasic::UpdateSources(OBSScene scene)
{
	...
	UpdateCustomVolCtrlSources(scene);
}

void OBSBasic::UpdateCustomVolCtrlSources(OBSScene scene)
{
	//新增vol控制台,要实现：串流时，切换镜头tab页面，也应该切换vol控制台的内容
	//清除旧的界面控件item
	ClearCustomVolumeControls();
	...
	obs_scene_enum_items(scene,
		[](obs_scene_t *scene, obs_sceneitem_t *item, void *p)
	{
		obs_source_t *pSource = obs_sceneitem_get_source(item);
		
		uint32_t     flags = obs_source_get_output_flags(pSource);
		if (flags & OBS_SOURCE_AUDIO)
		{
			OBSBasic *window = static_cast<OBSBasic*>(p);
			window->AddAudioSourceToCustomVolControl(pSource);
		}
...
void OBSBasic::CreatePropertiesWindow(obs_source_t *source, bool isNewSource)
{
	if (properties)
		properties->close();

	properties = new OBSBasicProperties(this, source);
	properties->SetIsNewSource(isNewSource);
...
void OBSBasic::AddScene(OBSSource source)
{
	...
	ui->sceneTabBar->addTab(QT_UTF8(name));

	ui->sceneTabBar->resize(140 * ui->sceneTabBar->count(), 40);

	// add button
	if ( ui->sceneTabBar->count() >= 4 )
	{
		ui->tabAddSceneBtn->setVisible(false);
	}
...
void OBSBasic::RemoveScene(OBSSource source)
{
	...
	{
		if ( ui->sceneTabBar->tabText(i) == QT_UTF8(name.c_str()) )
		{
			ui->sceneTabBar->removeTab(i);
			break;
		}
	}
	...
	if ( ui->sceneTabBar->count() < 4 )
	{
		ui->tabAddSceneBtn->setVisible(true);
	}
...
void OBSBasic::UpdateSceneSelection(OBSSource source)
{
	if (source) {
		...
		// tabbar
		for (int i = 0; i < ui->sceneTabBar->count(); ++i) {
			if (ui->sceneTabBar->tabText(i) == QT_UTF8(name)) {
				ui->sceneTabBar->setCurrentIndex(i);
				break;
			}
		}
	}
...
void OBSBasic::RenameSources(QString newName, QString prevName)
{
	...
	for (std::map<OBSSource, VolControlCustom*>::iterator it = m_volumeCustomsMap.begin(); it != m_volumeCustomsMap.end(); it++)
	{
		if (it->second->GetName().compare(prevName) == 0)
...
void OBSBasic::ActivateAudioSource(OBSSource source)
{
	...
	//新的音频控制
	//参考上面的对应都单独维护
	AddAudioSourceToCustomVolControl(source);

}
void OBSBasic::AddAudioSourceToCustomVolControl(OBSSource source)
{
	//避免重复添加，因为我们的自定义控件和既有的不一样，我们在串流时也会随着镜头的切换而刷新填充，我们自己填充+obs底层触发填充，会导致重复，故要避免
	if (m_volumeCustomsMap.find(source) == m_volumeCustomsMap.end())
	{
		//新的音频控制
		//参考上面的对应都单独维护
		VolControlCustom *volCustom = new VolControlCustom(source);

		connect(volCustom, &VolControlCustom::ConfigClicked,
			this, &OBSBasic::VolControlContextMenu);

		m_volumeCustomsMap[source] = volCustom;
		m_pHoverVolCtrlWidget->AddWidget2Layout(volCustom);
	}
...
void OBSBasic::DeactivateAudioSource(OBSSource source)
{
	...
	//新增vol控制台
	VolControlCustom *volControlCustom;
	std::map<OBSSource, VolControlCustom*>::iterator it = m_volumeCustomsMap.find(source);
	if (it != m_volumeCustomsMap.end())
	{
		volControlCustom = it->second;
		delete volControlCustom;
		m_volumeCustomsMap.erase(it);
	}
...
void OBSBasic::on_actionAddScene_triggered()
{
	...
	{
		string name = QT_TO_UTF8(sceneName);
		obs_source_t *source = obs_get_source_by_name(name.c_str());
		if (source) {
			QMessageBox::information(this,
					QTStr("NameExists.Title"),
			...
			obs_source_release(source);
			//weiqi.mwq：这里为何又进入到on_actionAddScene_triggered？不会死循环吗
...
void OBSBasic::CreateSourcePopupMenu(QListWidgetItem *item, bool preview)
{
	{// Filter audio source
		if ( item )
		{
			...
			if ( source )
			{
				std::string sname = obs_source_get_id(source);
				if (sname == "wasapi_output_capture" || sname == "wasapi_input_capture")
				{
					return;
				}
			}
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "tblive/logs") <= 0)
		return nullptr;
...
void OBSBasic::StreamDelayStarting(int sec)
{
	...
	statusbar->StreamDelayStarting(sec);
...
void OBSBasic::StreamDelayStopping(int sec)
{
	...
	statusbar->StreamDelayStopping(sec);
}
...
void OBSBasic::StreamingStart()
{
	...
	ui->streamButton->setText(QTStr("Basic.Main.StopStreaming"));
	ui->streamButton->setEnabled(true);
	statusbar->StreamStarted(outputHandler->streamOutput);
...
void OBSBasic::StreamingStop(int code)
{
	...
	else
	{
		if (m_bPauseClicked)
		{
			// Stop by pause, show restart button
			ui->pauseStreamBtn->setEnabled(true);
			ui->pauseStreamBtn->setVisible(false);

			ui->restartStreamBtn->setVisible(true);

			ui->bizWebview->SetStreamingStatus(TSS_PAUSE);

			m_bPauseClicked = false;
		}
		else if (m_bRestartCliecked)
		{
			// restart failed
			ui->restartStreamBtn->setEnabled(true);
			m_bRestartCliecked = false;
		...
		else
		{
			...
			m_bIsStartedOnceBefore = false;

			ui->pauseStreamBtn->setVisible(false);
			ui->restartStreamBtn->setVisible(false);
			ui->stopStreamBtn->setVisible(false);

			ui->bizWebview->SetStreamingStatus(TSS_STOP);
		}
	}
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "tblive");
	if (ret <= 0)
		return;
...
void OBSBasic::UpdateTitleBar()
{
	...
	{
		name << " - ";
		name << base::WideToUTF8(user.GetString());
	}
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	ret = GetConfigPath(profiles_path, 512, "tblive/basic/profiles");
	if (ret <= 0)
		return ret;
