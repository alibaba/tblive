		...
		const char *oldName = nullptr)
{
	...
	ret = GetConfigPath(path, sizeof(path), "tblive/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
		...
		const char *init_text)
{
	...
	char newPath[512];
	int ret = GetConfigPath(newPath, 512, "tblive/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
void OBSBasic::DeleteProfile(const char *profileName, const char *profileDir)
{
	...
	int ret = GetConfigPath(basePath, 512, "tblive/basic/profiles");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return;
	}
