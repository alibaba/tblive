...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "tblive/portrait") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "tblive/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "tblive/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "tblive/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "tblive/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif
	if (GetConfigPath(path, sizeof(path), "tblive/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "tblive/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "tblive/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "tblive/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "tblive/basic/profiles") <= 0)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "tblive/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
bool OBSApp::OBSInit()
{
	...
	if (licenseAccepted) {
		...
		//诡异：这里必须改变下窗口大小，layout才会正常工作（否则：标题栏hover不生效、addsence显示异常），所以这里主动触发一下
...
{
	BPtr<char>       logDir(GetConfigPathPtr("tblive/logs"));
	struct os_dirent *entry;
	os_dir_t         *dir        = os_opendir(logDir);
...
static void create_log_file(fstream &logFile)
{
	...
	dst << "tblive/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file("tblive/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("tblive/profiler_data/"));
...
#define CRASH_MESSAGE \
	...
	"%appdata%\\tblive\\crashes directory)"
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	delete_oldest_file("tblive/crashes");

	string name = "tblive/crashes/Crash ";
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "tblive/basic/profiles");
...
#if defined(OS_WIN)
void SetFocusToExistTblive()
{
	HWND hwnd = ::FindWindowEx(HWND_MESSAGE, NULL, tblive_cmd_class_name, NULL);
	if (NULL != hwnd)
	{
		wchar_t strCmd[] = L"tblive://set_focus";
		COPYDATASTRUCT tmpData;
		ZeroMemory(&tmpData, sizeof(tmpData));
		tmpData.dwData = EXTERNAL_CMD;
		tmpData.cbData = (DWORD)(_tcslen(strCmd) + 1)*sizeof(TCHAR);
		tmpData.lpData = (PVOID)strCmd;
...
int main(int argc, char *argv[])
{
...
#if defined(OS_WIN)
	std::wstring strUniqueMutex = L"TBLive{9253996F-636A-4D02-901D-CB61099639A0}";
	HANDLE uniqueMutex = ::CreateMutex(NULL, TRUE, strUniqueMutex.c_str());
	int nRet = ::GetLastError();
	if (uniqueMutex && nRet == ERROR_ALREADY_EXISTS)
	{
		SetFocusToExistTblive();

		// tblive already exist
	...
	char path[512] = { 0 };
	int pathLen = GetConfigPath(path, sizeof(path), "tblive/profile/authsdk");
	if (pathLen > 0)
	{
		dataDir = base::FilePath(std::string(path, pathLen));
	}
