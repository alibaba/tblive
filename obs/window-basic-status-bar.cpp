...
OBSBasicStatusBar::OBSBasicStatusBar(OBSBasic* pOBSBasic)
	:QStatusBar(pOBSBasic),
	delayInfo(new QLabel),
	droppedFrames(new QLabel),
	sessionTime(new QLabel),
	cpuUsage(new QLabel),
	kbps(new QLabel),
	m_pOBSBasic(pOBSBasic)
{
...
void OBSBasicStatusBar::Deactivate()
{
	...
	if (!m_pOBSBasic->outputHandler->Active()) {
		delete refreshTimer;
		sessionTime->setText(QString("00:00:00"));
		delayInfo->setText("");
...
void OBSBasicStatusBar::UpdateCPUUsage()
{
	...
	QString text;
	text += QString("CPU: ") +
		QString::number(m_pOBSBasic->GetCPUUsage(), 'f', 1) + QString("%");
...
{
	if (!m_pOBSBasic || !m_pOBSBasic->outputHandler)
		return;

	streamOutput = m_pOBSBasic->outputHandler->streamOutput;
