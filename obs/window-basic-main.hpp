...
enum eDragMode
{
	Drag_Undefine = 0,
	Drag_ResizeNorth,
	Drag_ResizeNorthEast,
	Drag_ResizeEast,
	Drag_ResizeSouthEast,
	Drag_ResizeSouth,
	Drag_ResizeSouthWest,
	Drag_ResizeWest,
	Drag_ResizeNorthWest,
	Drag_Move
};

class VolHoverWidget;
class VolControlCustom;
...
private:
	...
	OBSBasicStatusBar *statusbar = nullptr;
	...
	bool m_bServiceInited = false;
	bool m_bBizWebviewInited = false;
	QWebEngineView * m_hideWebview = nullptr;
	OBSSource m_streamingScene = nullptr;
	int m_curNewSceneNameIndex = 1;
	bool m_bPauseClicked = false;
	bool m_bRestartCliecked = false;
	bool m_bIsStartedOnceBefore = false;

private:
	bool		  HitTestDrag(QPoint mouseWndPos, long *result);
	void		  InitHoverVolWidget();
	...
	void		  ClearCustomVolumeControls();
	...
	void UpdateCustomVolCtrlSources(OBSScene scene);
	...
	void CreatePropertiesWindow(obs_source_t *source, bool isNewSource = false);
	...
	//´°¿Úmove¡¢resize
	QPoint	m_oldMouseGlobalPos;
	QRect	m_oldWndRct;
	int m_resizeBorderWidth;
	eDragMode m_nDragMode;
	...
	void InitServiceData(base::Closure callback);
	void InitBizWebview();
	void BringToTop();
	void InitTbliveUI();
	void SetTabStreamingStatus(QString sceneName);
	void CurrentTabSceneUpdateControls();

	void AddAudioSourceToCustomVolControl(OBSSource source);
	void ImpInstallChildrenEventFilter(QWidget *pW);
public slots:
	void StartStreaming();
	void StopStreaming();
...
private slots:
	...
	void CurrentTabSceneChanged(int idx);
	void OBSInitFinish();
...
private:
	...
	void AddSourceWithProperty(const char *id);
...
public:
	...
	void ReloadServiceFromConfig(std::string json);
	void TBLiveNeedUpdate(std::string ver, std::string msg, std::string download_url);
...
private slots:
	...
	void on_pushbt_main_set_clicked();
	...
	void on_selectBlackMagic_clicked();
    void on_selectCamera_clicked();
    void on_selectLocalVideo_clicked();
	void on_selectPic_clicked();
    void on_selectWindow_clicked();
	void on_selectMonitor_clicked();
	void on_tabAddSceneBtn_clicked();
	void on_switchSceneBtn_clicked();
	void on_startStreamBtn_clicked();
	void on_pauseStreamBtn_clicked();
	void on_restartStreamBtn_clicked();
	void on_stopStreamBtn_clicked();

	void on_volCtrlBtn_released();
...
private:
	...
	VolHoverWidget *m_pHoverVolCtrlWidget = nullptr;
	std::map<OBSSource, VolControlCustom*> m_volumeCustomsMap;
};
