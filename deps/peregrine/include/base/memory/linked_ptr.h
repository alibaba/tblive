...
// Forward declare
template<typename> class linked_ptr;
...
protected:
  ...
  enable_linked_from_this() : sp_link_this_(nullptr)
  {
  }

  enable_linked_from_this(enable_linked_from_this const &) : sp_link_this_(nullptr)
  {
  }

  enable_linked_from_this & operator=(enable_linked_from_this const &)
  {
    return *this;
  }
...
public:
  linked_ptr<T> linked_from_this()
  {
    if ( sp_link_this_ ) {
      return linked_ptr<T>( *sp_link_this_ );
    }
...
public:
  void _internal_accept_owner( linked_ptr<T> const * ppx ) const
  {
    sp_link_this_ = ppx;
  }
...
private:
  ...
  mutable linked_ptr<T> const* sp_link_this_;
};

namespace detail {

inline void sp_enable_linked_from_this(...) {
}

template< class X, class T > 
inline void sp_enable_linked_from_this( linked_ptr<X> const * ppx, enable_linked_from_this< T > const * pe )
{
  if( pe != 0 )
  {
...
class linked_ptr {
 public:
  ...
  explicit linked_ptr(T* ptr = NULL) { 
    capture(ptr); 
    detail::sp_enable_linked_from_this(this, ptr);
  }
  ~linked_ptr() { depart(); }
