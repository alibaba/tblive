...
// Splits an input of the form [<username>][":"<password>]"@"<host>[":"<port>] into its consitituent parts.
// Saves the result into |*username|, |*password|, |*host| and |*port|. If the input did not have
// the optional port, sets |*port| to -1, the optional port, sets |*username| to "", the optional port, sets |*password| to "".
// Returns true if the parsing was successful, false otherwise.
// The returned host is NOT canonicalized, and may be invalid. If <host> is
// an IPv6 literal address, the returned host includes the square brackets.
NET_EXPORT bool ParseHostPortAndUsernamePassword(
    std::string::const_iterator host_and_port_begin,
    std::string::const_iterator host_and_port_end,
    std::string* host,
    int* port,
